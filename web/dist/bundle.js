...
var HIGHLIGHT_TO_COLOR = [
  0,
  // HighlightNone -> Black
  9,
  // HighlightBackground -> BrBlack
  2,
...
var ALT_WHITE = 8;
var BR_BLACK = 9;
var HIGHLIGHT_BACKGROUND = 1;
var FLOOR_SYMBOL = 183;
var FLOOR_DIM = "\u22C5";
var NBSP = "\xA0";
function decodeCell(w) {
  return { code: w >>> 16, fg: w >>> 8 & 255, bg: w & 255 };
}
function isBright(fg) {
  return fg > BR_BLACK;
}
function resolveFg(fgRaw, row) {
  return row % 2 === 0 && fgRaw === WHITE ? ALT_WHITE : fgRaw;
}
function glyph(code, fg) {
  if (code === 32) return NBSP;
...
function highlightColor(bg) {
  return PALETTE[HIGHLIGHT_TO_COLOR[bg]];
}
function styledCell(w, row) {
